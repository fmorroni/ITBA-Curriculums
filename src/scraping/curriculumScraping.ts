...
export async function parseCurriculum(curriculumUrl, cookies) {
  const curriculumPageRes = await myFetch(curriculumUrl, { cookies })
  const { document } = new JSDOM(await curriculumPageRes.text(), { url: getBaseUrl(curriculumPageRes) }).window;
